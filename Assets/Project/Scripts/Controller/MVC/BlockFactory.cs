...
public class BlockFactory : MonoBehaviour
{
    ...
    public Dictionary<(int x, int y), BoardBlockObject> boardBlockDic;
    public Dictionary<(int, bool), BoardBlockObject> standardBlockDic = new Dictionary<(int, bool), BoardBlockObject>();
   
    public GameObject boardParent;

    public Dictionary<(int x, int y), BoardBlockObject> BBDic { get { return boardBlockDic; } }
    

    public async Task CreateBoardAsync(BoardController boardController)
    {
        ...
        GameObject prefab = await boardController.LoadPrefabAsync("Board Cell.prefab");
        ...
        for (int i = 0; i < stageDatas[stageIdx].boardBlocks.Count; i++)
        {
            ...
            GameObject blockObj = Instantiate(prefab, boardParent.transform);
            ...
            {
                boardBlock._ctrl = boardController;
                boardBlock.x = data.x;
                boardBlock.y = data.y;

                if (boardController.GetWCIDic.ContainsKey((boardBlock.x, boardBlock.y)))
                {
                    for (int k = 0; k < boardController.GetWCIDic[(boardBlock.x, boardBlock.y)].Count; k++)
                    {
                        boardBlock.colorType.Add(boardController.GetWCIDic[(boardBlock.x, boardBlock.y)].Keys.ElementAt(k).Item2);
                        boardBlock.len.Add(boardController.GetWCIDic[(boardBlock.x, boardBlock.y)].Values.ElementAt(k));

                        DestroyWallDirection dir = boardController.GetWCIDic[(boardBlock.x, boardBlock.y)].Keys.ElementAt(k).Item1;
        ...
        boardController.boardWidth = boardBlockDic.Keys.Max(k => k.x);
        boardController.boardHeight = boardBlockDic.Keys.Max(k => k.y);
    }
}
